...
class Coverage:
    ...
    def run(self):
        try:
            ...
            vertices = [[176, 77], [171, 78], [170, 79], [159, 82], [158, 83], [105, 100], [101, 102], [100, 105], [92, 117], [82, 134],
                        [12, 253], [5, 265], [3, 270], [6, 273], [14, 280], [29, 293], [60, 319], [141, 390], [149, 397], [151, 398], [178, 398],
                        [178, 76]]
            ...
            # plt.ylim(0, 10)
            plt.xlim(-100, 300)
            plt.plot([vertex[0] for vertex in vertices], [vertex[1] for vertex in vertices], label='Célula') 
            plt.scatter(centroid[0], centroid[1], s=2, c='r')       
            plt.legend()
            plt.savefig("cell_before.png")
            plt.close()

            for vertex in vertices:
                T = self.get_transform_matrix()
                vec = np.dot(T, np.array([vertex[0], vertex[1], 1, 1]))[:2]

                vertex[0] = vec[0] * (15 / 500)
                vertex[1] = vec[1] * (15 / 500)

            center = np.dot(T, np.array([centroid[0], centroid[1], 1, 1]))[:2]
            centroid[0] = center[0] * (15 / 500)
            centroid[1] = center[1] * (15 / 500)
            ...
            plt.scatter(centroid[0], centroid[1], s=2, c='r')       
            plt.legend()
            plt.savefig("cell_temp.png")
            plt.close()

            vertices = sorted(vertices, key=lambda x: x[1])

            # Including first point so the robot closes the polygon
            vertices.append(vertices[0])
            # cells.append([vertices, centroid])

            # Second cell
            vertices = [[182, 239], [182, 398], [198, 398], [198, 279], [196, 274], [192, 264], [184, 244], [182, 241]] 
            centroid = [189, 328]
            ...
            # Third cell
            vertices = [[249, 52], [243, 54], [231, 58], [186, 73], [182, 75],
                        [182, 227], [184, 226], [188, 224], [190, 222], [200, 217],
                        [260, 184], [262, 182], [265, 181], [267, 180], [268, 56],
                        [265, 56], [252, 52]] 
            ...
            for cell in cells:
                ...
                while not self.polygon_area(cell[0]) <= 0.5:
                    L = 0.45

                    # Cortar pontos muito próximos

                    for vertex in cell[0]:
                        centroid = cell[1]
                ...
                #     print("Área:")
                #     print(self.polygon_area(cell[0]))
